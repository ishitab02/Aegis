...
export function SentinelCard({ assessment }: { assessment: ThreatAssessment }) {
  const Icon = ICONS[assessment.sentinel_type] ?? ShieldCheck;
  const label = SENTINEL_LABELS[assessment.sentinel_type] ?? assessment.sentinel_type;
  ...
  return (
    <div className="rounded-xl border border-aegis-border bg-aegis-card p-5">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-gray-400 mt-2 line-clamp-2">{assessment.details}</p>
